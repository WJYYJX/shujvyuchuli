...
if __name__ == '__main__':
    path1 = r"F:\goubangzi2000new"  # 包含四类的文件夹路径
        ...
        for folder_name1 in os.listdir(path0):  # 获取其中一个子文件夹内的子文件夹名
            ...
            for folder_name2 in os.listdir(path2):  # 获取其中一个子文件夹内的子文件夹名
                ...
                for folder_name3 in os.listdir(path3):  # 获取其中一个子文件夹内的子文件夹名
                    path4 = os.path.join(path3, folder_name3)

                    try:
                        img_name_list = os.listdir(path4)  #
                    except FileNotFoundError:
                        print("没有该学生文件夹  ")

                    if len(img_name_list) != 12:
                        print(path4)
                        new_file_path = 'F:/errorleft300/' + folder_name0 + '/' + folder_name1 + '/' + folder_name2 + '/' + folder_name3 + '/' # 右眼保存路径
                        shutil.move(path4, new_file_path)
                        # print("文件夹中图像数目不对")
