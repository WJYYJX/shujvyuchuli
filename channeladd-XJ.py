...
if __name__ == '__main__':
    train_image = []
    train_label = []
    path1 = r"C:\Users\111\Desktop\gongbangzi_1.2.4.6.10.16\val"  # 包含四类的文件夹路径 C:\Users\111\Desktop\zhongzhuan\val
    ...
    for folder_name0 in os.listdir(path1):  # 一级文件夹里的子文件夹共分成四类
        ...
        for folder_name1 in os.listdir(path0):  # 获取其中一个子文件夹内的子文件夹名
            ...
            for folder_name2 in os.listdir(path2):  # 获取其中一个子文件夹内的子文件夹名
                ...
                # img1 = cv2.imread(path3 + '/1.bmp', cv2.COLOR_BGR2RGB)
                img2 = cv2.imread(path3 + '/002.jpg.bmp', cv2.COLOR_BGR2RGB)
                img3 = cv2.imread(path3 + '/003.jpg.bmp', cv2.COLOR_BGR2RGB)
                # img4 = cv2.imread(path3 + '/4.bmp', cv2.COLOR_BGR2RGB)
                img5 = cv2.imread(path3 + '/005.jpg.bmp', cv2.COLOR_BGR2RGB)
                # img6 = cv2.imread(path3 + '/6.bmp', cv2.COLOR_BGR2RGB)
                img7 = cv2.imread(path3 + '/007.jpg.bmp', cv2.COLOR_BGR2RGB)
                # img8 = cv2.imread(path3 + '/8.bmp', cv2.COLOR_BGR2RGB)
                img9 = cv2.imread(path3 + '/009.jpg.bmp', cv2.COLOR_BGR2RGB)
                # img10 = cv2.imread(path3 + '/10.bmp', cv2.COLOR_BGR2RGB)
                img11 = cv2.imread(path3 + '/011.jpg.bmp', cv2.COLOR_BGR2RGB)
                # img12 = cv2.imread(path3 + '/12.bmp', cv2.COLOR_BGR2RGB)
                ...
                savepath = r"F:\goubangzi_1.2.4.6.10.16\val"

                savepath1 = savepath + '/' + 'image' '/' + folder_name0 + '/'+ folder_name1 + '/'
                folder = os.path.exists(savepath1)
                if not folder:  # 判断是否存在文件夹如果不存在则创建为文件夹
                    os.makedirs(savepath1)  # makedirs 创建文件时如果路径不存在会创建这个路径

                savepath2 = savepath + '/' + 'label' '/' + folder_name0 + '/'+ folder_name1 + '/'
                ...
                try:
                    res = cv2.merge([img2, img3, img5, img7, img9, img11])
                    res = np.array(res)
