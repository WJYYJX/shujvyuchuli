...
#    print(imgs_num)
    ...
    # 保存图片
    pathl = 'F:/goubangzi2000new/' +  'left/' + folder_name6 +'/' + name0 +'/'  + folder_name1 +'/'# 左眼保存路径
    folder = os.path.exists(pathl)
    if not folder:  # 判断是否存在文件夹如果不存在则创建为文件夹
        os.makedirs(pathl)  # makedirs 创建文件时如果路径不存在会创建这个路径
    #for i in range(12):
    cv2.imwrite(pathl + str(folder_name2) + '.bmp',
                        img_cut_left[0])

    # pathr = 'F:/jinzhou1/' + '2/' + folder_name6 + '/'+ name0 +'/'  # 右眼保存路径
...
if __name__ == '__main__':
    ...
    path1 = "G:\\goubangzi_origin"  # 包含四类的文件夹路径
    for folder_name6 in os.listdir(path1):  # 一级文件夹里的子文件夹共分成四类
        path6 = os.path.join(path1, folder_name6)
        for folder_name0 in os.listdir(path6):  # 一级文件夹里的子文件夹共分成四类
            path0 = os.path.join(path6, folder_name0)
