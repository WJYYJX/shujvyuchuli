...
label_file = open("F:\\data\\屈光\\left_label.txt", 'r')
# 原始文件的根目录
#input_path = "C:\\Users\\111\\Desktop\\data_jinzhou\\筛选后\\宜州小学数据集\\宜州小学屈光度图像1（已去重）"
input_path = "F:\\xinjiangdata\\1"
# 保存文件的根目录
output_path = "D:\\datasets\\xinjiangleft"
# 标签数组
...
for line in data:
    ...
    # 新文件位置
        new_file_path = os.path.join(new_file_path+'\\'+file_label, file_name)
        print("" + str(i) + "\t正在将 " + old_file_path + " 复制到 " + new_file_path)
    # 复制文件
        try:
            #shutil.copyfile(old_file_path+'.zip', new_file_path+'.zip')
            shutil.copytree(old_file_path , new_file_path,symlinks=False, ignore=None)
        except:
            print('fail to read {}.jpg',format(old_file_path))
            continue
